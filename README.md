...
1.  **Transport Layer (Dumb Pipe):** Слой WebSocket не знает бизнес-логики. Он принимает "сырой" JSON и передает его дальше.
2.  **IncomingRawMessage:** Данные оборачиваются в строго типизированный Dataclass.
3.  **Dispatcher:** Центральный узел, который анализирует тип события.
4.  **Handlers:** Изолированные обработчики, каждый из которых отвечает за свой тип задач (валидация схемы через Pydantic происходит здесь).
...
Полный текст лицензии доступен в файле [LICENSE](https://github.com/cnvuls/dozorniy/master/LICENSE.md).
