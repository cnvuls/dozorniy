...
class LogStyle(TypedDict):
    icon: ft.IconData
    color: ft.ColorValue


LOG_STYLES: dict[type[BaseLogEvent], LogStyle] = {
    ErrorLogEvent: {"color": ft.Colors.ERROR, "icon": ft.Icons.ERROR_OUTLINE},
    ConsoleLogEvent: {"color": ft.Colors.PRIMARY, "icon": ft.Icons.TERMINAL},
    InfoLogEvent: {
        "color": ft.Colors.SECONDARY,
        "icon": ft.Icons.NOTIFICATIONS_OUTLINED,
    },
    BaseLogEvent: {"color": ft.Colors.ON_SURFACE, "icon": ft.Icons.INFO_OUTLINE},
}
...
class ListLog(ft.Container):
    def __init__(self, bus: EventBus):
        ...
        self._log_history: deque = deque(maxlen=self.max_log_lines)
    ...
    def _add_to_ui(self, event: BaseLogEvent):
        ...
        item = ft.ListTile(
            ...
            subtitle=ft.Text(
                f"[{time_str}] Source: {event.source}", size=12, color=ft.Colors.OUTLINE
            ),
            dense=True,
        )
